...
class PickitSdk {
    ...
    public function get_final_budget(\WC_Order $order){
        ...
        if ("pickit" !== $shipping_method->get_method_id()){
            ...
            $settings = Helper::get_setup_from_settings();
            $retailer = [];
            $retailer['postalCode'] = $settings['pickup-postalcode-dflt'];
            $retailer['address'] = $settings['pickup-address-dflt'] . " " . $settings['pickup-address-nbr-dflt'] ;
            $retailer['city'] = $settings['pickup-city-dflt'];
            $retailer['province'] = $settings['pickup-state-dflt'];
            ...
            $retailer = [];
            $retailer['postalCode'] = $shipping_method_pickit->pickup_postalcode;
            $retailer['address'] = $shipping_method_pickit->pickup_address . " " . $shipping_method_pickit->pickup_address_nbr;
            $retailer['city'] = $shipping_method_pickit->pickup_city;
            $retailer['province'] = $shipping_method_pickit->pickup_state;
        }       
    ...
    public function create_order(\WC_Order $order, $uuid){
        ...
        if ("pickit" === $shipping_method->get_method_id()){
            ...
            $operationType = "ST";
            $point = 0;        
            $curier = $shipping_method->get_method_id();  
        }
